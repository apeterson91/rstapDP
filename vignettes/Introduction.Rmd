...
estimated in those models were estimated to be homogenous at the level of the entire population; It was assumed in the model formulation that all individuals have the same effect of living near a BEF.The `rstapDP` package removes this assumption by
allowing for heterogeneity in the STAP effect across the population. 

## Illustrative Example

For example, consider a sample of individuals near Fast Food Restaurants (FFRs) where half of the subjets are at "high risk"
of having an average increase in their BMI as a consequence of living near the FFR.
...
or the number of risk populations requires the estimation of some number of STAP effects $f_i(\mathcal{D}_i)$ where $f(\cdot)$ is the (say) spatial exposure function relating how the average of health outcome $Y_i$ increases as a function of where a new FFR is placed relative to the subjects.



The STAP-DP model which `rstapDP` impliments incorporates these ideas in a regression setting as follows:

$$
E[Y_i] =  \mathbf{Z}_i^{T} \mathbf{\delta} + f_i(\mathcal{D}_i) \quad i = 1,...,n.
$$

Above, $f_i(\mathcal{D})$, the smooth function of BEF exposure distances is modeled through
spline regression coefficients, $\mathbf{\beta}_i$ similar to the `rsstap` package. However, in the `rstapDP` package, we place a Dirichlet Process (DP)  prior on these regression coefficients:
...
With our model fitting complete, our first check will be to look at convergence diagnostics on the residual variance and concentration parameter as well as the posterior predictive checks via the `ppc` function.
...
Finally, we should take at look at the estimated and true exposure curves.
